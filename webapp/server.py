...
from training.feature_extractor import SplendorFeatureExtractor  # noqa: F401 — register for checkpoint deserialization
...
def return_gems():
    ...
    # ---- Switch turn to AI (bypasses env.step, must do manually) ----
    state.current_player = 1 - human_idx
    state.turn_number += 1
    session["obs"] = build_observation(state, state.current_player)
    session["mask"] = get_action_mask(state, state.current_player)
